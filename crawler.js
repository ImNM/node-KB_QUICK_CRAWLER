...
const KB_QUICK_URL = 'https://obank.kbstar.com/quics?page=C025255&cc=b028364:b028702#loading';
...
await page.evaluate(() => {
   ...
   return imgurl;
    ...
    await page2.screenshot({ 
        ...
        //이미지 추출 과정
        page2.close();
        await sharp(PATH_KEYBOARD_IMG).extract({left:75, top:100, width:55, height:55 }).toFile("./assets/top.png")
        await sharp(PATH_KEYBOARD_IMG).extract({left:17, top:158, width:55, height:55 }).toFile("./assets/left.png")
        await sharp(PATH_KEYBOARD_IMG).extract({left:75, top:158, width:55, height:55 }).toFile("./assets/center.png")
        await sharp(PATH_KEYBOARD_IMG).extract({left:133, top:158, width:55, height:55 }).toFile("./assets/right.png")
        await sharp(PATH_KEYBOARD_IMG).extract({left:75, top:216, width:55, height:55 }).toFile("./assets/bottom.png")
    })
}).then(()=>{ // 추출된 이미지 인식
...
(async ()=> {
  ...
  var { data: { text } }= await worker.recognize('./assets/top.png');
  console.log("top",parseInt(text));
  button.push(parseInt(text))
  var { data: { text } }= await worker.recognize('./assets/left.png');
  console.log("left",parseInt(text));
  button.push(parseInt(text))
  var { data: { text } }= await worker.recognize('./assets/center.png');
  console.log("center",parseInt(text));
  button.push(parseInt(text))
  var { data: { text } }= await worker.recognize('./assets/right.png');
  console.log("right",parseInt(text));
  button.push(parseInt(text))
  var { data: { text } }= await worker.recognize('./assets/bottom.png');
  ...
  return button
    ...
    await page.evaluate((inpumNumber,accountAddress,accountId) => {
        ...
        document.querySelector("#pop_contents > div.btnArea > span > input[type=button]").click();
